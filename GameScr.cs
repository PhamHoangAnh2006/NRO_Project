...
public class GameScr : mScreen, IChatable
{
	...
	public bool xskill = true;

	public static bool anhat;

	public static bool chiso;
	...
	public static long smpet;

	public static bool autoexit;

	public static bool updatekhu;

	public static bool showCharinfo;

	public static bool showPetinfo;
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						{
							Service.gI().openUIZone1();
							info1.addInfo("Mở Khu Nhanh", 0);
						}
						else if (GameCanvas.keyAsciiPress == 116)
						{
							isAutoPlay = !isAutoPlay;
							canAutoPlay = !canAutoPlay;
							info1.addInfo("Tàn Sát", 0);
						}
						else
						{
							...
							{
								MyVector myVector = new MyVector();
								myVector.addElement(new Command(showCharinfo ? "Thông Tin Sư Phụ : Bật" : "Thông Tin Sư Phụ : Tắt", 9));
								myVector.addElement(new Command(showPetinfo ? "Thông Tin Đệ Tử : Bật" : "Thông Tin Đệ Tử : Tắt", 10));
								myVector.addElement(new Command(viewboss ? "Hiện Thông Tin BOSS: Bật" : "Hiện Thông Tin BOOS: Tắt", 3));
								myVector.addElement(new Command(chiso ? "Chỉ Số Tăng: Bật" : "Chỉ Số Tăng: Tắt", 4));
								myVector.addElement(new Command(anhat ? "Auto Nhặt: Bật" : "Auto Nhặt: Tắt", 5));
								myVector.addElement(new Command(lineboss ? "Tạo Đường Tới BOSS: Bật" : "Tạo Đường Tới BOSS: Tắt", 6));
								myVector.addElement(new Command(canAutoPlay ? "Tàn Sát: Bật" : "Tàn Sát: Tắt", 7));
								myVector.addElement(new Command(updatekhu ? "Update Khu : Bật" : "Update Khu : Tắt", 8));
				...
				else
				{
					...
					{
						myVector2.addElement(array[j] + string.Empty);
					}
					...
					{
						keyInput += ((string)myVector2.elementAt(k)).ToUpper();
					}
	...
	private void askToPick()
	{
		...
		ChatPopup.addChatPopupWithIcon(nhatvatpham, 1000000, npc, 5820);
	}
	...
	public override void update()
	{
		...
		if (xskill && GameCanvas.gameTick % 20 == 0)
		{
		...
		try
		{
			...
			for (int l = 0; l < vMob.size(); l++)
			{
				((Mob)vMob.elementAt(l)).update();
			}
			GameCanvas.debug("E6", 0);
			for (int m = 0; m < vNpc.size(); m++)
			{
				((Npc)vNpc.elementAt(m)).update();
			}
			...
			{
				int num8 = dHP - Char.myCharz().cHP >> 1;
				if (num8 < 1)
				{
					num8 = 1;
				}
			...
			{
				int num9 = dMP - Char.myCharz().cMP >> 1;
				if (num9 < 1)
				{
					num9 = 1;
				}
			...
			for (int num11 = 0; num11 < Char.vItemTime.size(); num11++)
			{
				((ItemTime)Char.vItemTime.elementAt(num11)).update();
			}
			for (int num12 = 0; num12 < textTime.size(); num12++)
			{
				((ItemTime)textTime.elementAt(num12)).update();
			}
			updateChatVip();
		}
		catch (Exception)
	...
	public override void paint(mGraphics g)
	{
				...
				if (!viewboss)
				{
					try
					{
						...
						foreach (string item in listboss.AsEnumerable().Reverse())
						{
							...
							mFont mFont2 = ((!array[1].Trim().Contains(TileMap.mapName)) ? mFont.tahoma_7b_white : mFont.tahoma_7_red);
							mFont2.drawString(g, array[0].Replace("BOSS ", "") + array[1].Replace("zone", "khu") + " - " + ((num2 < 60) ? (num2 + " giây") : (timeSpan.Minutes + " phút")) + " trước", 500, GameCanvas.h - num, mFont.RIGHT, mFont.tahoma_7b_yellow);
							num -= 10;
						}
					}
					catch
		...
		for (int num3 = 0; num3 < vMob.size(); num3++)
		{
			((Mob)vMob.elementAt(num3)).paint(g);
		}
		for (int num4 = 0; num4 < Teleport.vTeleport.size(); num4++)
		{
			((Teleport)Teleport.vTeleport.elementAt(num4)).paint(g);
		}
		for (int num5 = 0; num5 < vCharInMap.size(); num5++)
		{
			Char char3 = null;
			try
			{
				char3 = (Char)vCharInMap.elementAt(num5);
			}
			catch (Exception)
			{
			}
		...
		for (int num7 = 0; num7 < vItemMap.size(); num7++)
		{
			((ItemMap)vItemMap.elementAt(num7)).paint(g);
		}
		g.translate(0, -GameCanvas.transY);
		...
		paintEffect(g);
		paintBgItem(g, 3);
		for (int num8 = 0; num8 < vNpc.size(); num8++)
		{
			((Npc)vNpc.elementAt(num8)).paintName(g);
		}
		EffecMn.paintLayer3(g);
		for (int num9 = 0; num9 < vNpc.size(); num9++)
		{
			Npc npc2 = (Npc)vNpc.elementAt(num9);
			if (npc2.chatInfo != null)
			{
				npc2?.chatInfo.paint(g, npc2.cx, npc2.cy - npc2.ch - GameCanvas.transY, npc2.cdir);
			}
		}
		for (int num10 = 0; num10 < vCharInMap.size(); num10++)
		{
			Char char5 = null;
			try
			{
				char5 = (Char)vCharInMap.elementAt(num10);
			}
			catch (Exception)
			{
			}
		...
		for (int num11 = 0; num11 < TileMap.vCurrItem.size(); num11++)
		{
			BgItem bgItem = (BgItem)TileMap.vCurrItem.elementAt(num11);
		...
		bool flag = true;
		for (int num12 = 0; num12 < BackgroudEffect.vBgEffect.size(); num12++)
		{
			if (((BackgroudEffect)BackgroudEffect.vBgEffect.elementAt(num12)).typeEff == 0)
			{
				flag = false;
				break;
			}
		...
		{
			int num13 = TileMap.pxw / (mGraphics.getImageWidth(TileMap.imgLight) + 50);
			if (num13 <= 0)
			{
				num13 = 1;
			}
			if (TileMap.tileID != 28)
			{
				...
				{
					int num15 = 100 + num14 * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2;
					int num16 = -20;
					int imageWidth = mGraphics.getImageWidth(TileMap.imgLight);
					if (num15 + imageWidth >= cmx && num15 <= cmx + GameCanvas.w && num16 + mGraphics.getImageHeight(TileMap.imgLight) >= cmy && num16 <= cmy + GameCanvas.h)
					{
						g.drawImage(TileMap.imgLight, 100 + num14 * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2, num16, 0);
					}
				}
			}
		}
		...
		if (lineboss)
		{
			...
			{
				Char char6 = (Char)vCharInMap.elementAt(num17);
				if (char6.cTypePk == 5)
				{
					g.drawLine(Char.myCharz().cx - cmx, Char.myCharz().cy - cmy, char6.cx - cmx, char6.cy - cmy);
				}
			}
		}
		if (isPaintOther)
		...
		mFont.tahoma_7.drawString(g, "MOD BY : HOÀNG ANH ", 240, 12, mFont.CENTER, mFont.tahoma_7b_white);
		mFont.tahoma_7.drawString(g, "Thời gian : " + DateTime.Now, 70, GameCanvas.h - 250, mFont.CENTER, mFont.tahoma_7b_white);
		mFont.tahoma_7.drawString(g, "Map :" + TileMap.mapName + "[" + TileMap.mapID + "]" + "-Khu :" + "[" + TileMap.zoneID + "]", 70, GameCanvas.h - 240, mFont.CENTER, mFont.tahoma_7b_white);
		if (showCharinfo)
		{
			mFont.tahoma_7b_white.drawString(g, "Sư Phụ:", 60, GameCanvas.h - 120, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, string.Concat(new object[2]
			{
				"Vàng:",
				Char.myCharz().xu
			}), 50, GameCanvas.h - 90, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, string.Concat(new object[2]
			{
				"Ngọc:",
				Char.myCharz().luong
			}), 50, GameCanvas.h - 80, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, "HP : " + NinjaUtil.getMoneys(Char.myCharz().cHP) + " (" + Mathf.Round((float)Char.myCharz().cHP / (float)Char.myCharz().cHPFull * 100f) + "%" + ")", 50, GameCanvas.h - 110, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, "MP : " + NinjaUtil.getMoneys(Char.myCharz().cMP) + " (" + Mathf.Round((float)Char.myCharz().cMP / (float)Char.myCharz().cMPFull * 100f) + "%" + ")", 50, GameCanvas.h - 100, mFont.CENTER, mFont.tahoma_7b_white);
		}
		if (showPetinfo)
		{
			mFont.tahoma_7b_white.drawString(g, "Đệ Tử:", 140, GameCanvas.h - 130, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, string.Concat(new string[2]
			{
				"SM : ",
				NinjaUtil.getMoneys(Char.myPetz().cHP)
			}), 140, GameCanvas.h - 120, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, "HP : " + NinjaUtil.getMoneys(Char.myPetz().cHP) + " (" + Mathf.Round((float)Char.myPetz().cHP / (float)Char.myPetz().cHPFull * 100f) + "%" + ")", 140, GameCanvas.h - 110, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, "MP : " + NinjaUtil.getMoneys(Char.myPetz().cMP) + " (" + Mathf.Round((float)Char.myPetz().cMP / (float)Char.myPetz().cMPFull * 100f) + "%" + ")", 140, GameCanvas.h - 100, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, string.Concat(new string[2]
			{
				"SĐ : ",
				NinjaUtil.getMoneys(Char.myPetz().cDamFull)
			}), 140, GameCanvas.h - 90, mFont.CENTER, mFont.tahoma_7b_white);
			mFont.tahoma_7.drawString(g, string.Concat(new string[2]
			{
				"TN : ",
				NinjaUtil.getMoneys(Char.myPetz().cTiemNang)
			}), 140, GameCanvas.h - 80, mFont.CENTER, mFont.tahoma_7b_white);
		}
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num3; i++)
				{
					if (Main.isPC)
					{
						...
						if (TField.isQwerty)
						{
							string[] obj = new string[8] { "1", "2", "3", "4", null, null, null, null };
							array2 = obj;
							obj[4] = "5";
							obj[5] = "6";
							obj[6] = "7";
						...
						mFont.tahoma_7b_dark.drawString(g, array3[i], xSkill + xS[i] + 14, yS[i] - 13, mFont.CENTER);
						mFont.tahoma_7b_white.drawString(g, array3[i], xSkill + xS[i] + 14, yS[i] - 14, mFont.CENTER);
					}
					Skill skill = array[i];
					...
					long num4 = skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill;
					mFont.tahoma_7_white.drawString(g, (num4 > 0) ? string.Concat(num4 / 1000) : string.Empty, xSkill + xS[i] + 14, yS[i] + 8, mFont.CENTER, mFont.tahoma_7b_red);
				}
			}
			paintGamePad(g);
		}
	...
	{
		if (text.Contains("upkhu"))
		{
			updatekhu = !updatekhu;
			new Thread(Updatekhu).Start();
			info1.addInfo("Update Khu Theo Thời Gian Thực:" + (updatekhu ? "Bật" : "Tắt"), 0);
			text = string.Empty;
		}
		if (text.Contains("pet "))
		{
			smpet = long.Parse(text.Replace("pet ", ""));
			GameCanvas.startOKDlg("Sức Mạnh Đệ Tử Max Tự Out Game Là " + smpet.ToString("#,000") + " Sức Mạnh");
			text = string.Empty;
		}
		if (smpet != 0L && text == "out")
		{
			autoexit = !autoexit;
			new Thread(autoout).Start();
			GameCanvas.startOKDlg("Tự Động Out Game Khi Đệ Đủ Sức Mạnh " + (autoexit ? " - Bật" : " - Tắt"));
			text = string.Empty;
		}
		else if (smpet == 0L && text == "out")
		{
			GameCanvas.startOKDlg("Bạn Chưa Nhập Đủ Sức Mạnh Của Đệ Tử !");
			text = string.Empty;
		}
		if (text == "sb")
		{
			viewboss = !viewboss;
			info1.addInfo("Hiển Thị Thông Tin Boss:" + (viewboss ? "Bật" : "Tắt"), 0);
			text = string.Empty;
		}
		if (text == "line")
		{
			lineboss = !lineboss;
			info1.addInfo("Tạo đường kẻ tới boss :" + (lineboss ? "Bật" : "Tắt"), 0);
			text = string.Empty;
		}
		if (text == "cs")
		{
			chiso = !chiso;
			new Thread(autocs).Start();
			info1.addInfo("Chỉ số tăng :" + (chiso ? "Bật" : "Tắt"), 0);
			text = string.Empty;
		}
		if (text == "csvip")
		{
			chiso = !chiso;
			new Thread(autocs1).Start();
			info1.addInfo("Chỉ số tăng :" + (chiso ? "Bật" : "Tắt"), 0);
			text = string.Empty;
		}
		if (text == "tl")
		{
			...
			info1.addInfo("Qua Tương Lai ", 0);
		}
		if (text.Contains("anhat"))
		{
			anhat = !anhat;
			new Thread(autonhat).Start();
			info1.addInfo("Auto nhặt:" + (anhat ? "Bật " : "Tắt"), 0);
			text = string.Empty;
		}
		if (text.Contains("speed"))
		{
			Time.timeScale = int.Parse(text.Replace("speed", ""));
			info1.addInfo("Tăng tốc game :" + Time.timeScale, 0);
			text = string.Empty;
			return;
		}
		if (text.Contains("k_"))
		{
			Service.gI().requestChangeZone(int.Parse(text.Split()[1]), -1);
			Time.timeScale = int.Parse(text.Replace("k_", ""));
			text = string.Empty;
			return;
		}
		Res.outz("CHAT");
	...
	public void giaodich(int playerID)
	{
		...
		if (@char != null)
		{
			startYesNoPopUp(@char.cName + mResources.want_to_trade, new Command(mResources.YES, 11114, @char), new Command(mResources.YES, 2009, @char));
		}
	}
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 11113:
		{
			...
			if (char3 != null)
			{
				Service.gI().giaodich(0, char3.charID, -1, -1);
			}
			return;
		}
		case 11114:
		...
		{
			object[] obj = (object[])p;
			Skill skill2 = (Skill)obj[0];
			int num = int.Parse((string)obj[1]);
			for (int i = 0; i < keySkill.Length; i++)
			{
				if (keySkill[i] == skill2)
				{
					keySkill[i] = null;
				}
			}
		...
		if (idAction == 9)
		{
			...
			info1.addInfo("Thông Tin Sư Phụ : " + (showCharinfo ? "Bật" : "Tắt"), 0);
		}
		if (idAction == 10)
		{
			showPetinfo = !showPetinfo;
			info1.addInfo("Thông Tin Đệ Tử : " + (showPetinfo ? "Bật" : "Tắt"), 0);
		}
		switch (idAction)
		{
		case 11059:
		{
			Skill skill4 = onScreenSkill[selectedIndexSkill];
	...
	public void chatVip(string chatVip)
	{
		...
		if (!viewboss && chatVip.StartsWith("BOSS"))
		{
			if (listboss.Count > 5)
			{
				listboss.RemoveAt(0);
			}
			listboss.Add(chatVip.Replace("BOSS Thần thiên sứ 01 vừa xuất hiện tại", "") + "-" + DateTime.Now.ToString("HH:mm:ss"));
			listboss.Add(chatVip.Replace("BOSS Thần thiên sứ 02 vừa xuất hiện tại", "") + "-" + DateTime.Now.ToString("HH:mm:ss"));
			listboss.Add(chatVip.Replace("BOSS Thần hủy diệt vừa xuất hiện tại", "") + "-" + DateTime.Now.ToString("HH:mm:ss"));
		}
	...
	public static void capsule()
	{
		try
		{
			...
			{
				if (Char.myCharz().arrItemBag[b].template.id == 194)
				{
	...
	public static void capsule1()
	{
		try
		{
			for (sbyte b = 0; b < Char.myCharz().arrItemBag.Length; b = (sbyte)(b + 1))
			{
				if (Char.myCharz().arrItemBag[b].template.id == 193)
				{
					Service.gI().useItem(0, 1, b, -1);
					break;
				}
	...
	public void autocs()
	{
		string text = new StreamReader(new FileStream("chiso.txt", FileMode.Open), Encoding.UTF8).ReadToEnd();
		int num = int.Parse(text.Split('|')[0]);
		int num2 = int.Parse(text.Split('|')[1]);
		int num3 = int.Parse(text.Split('|')[2]);
		while (chiso)
		{
			if (Char.myCharz().cHPGoc <= num - 2000)
			{
			...
			if (Char.myCharz().cHPGoc <= num - 20)
			{
				Service.gI().upPotential(0, 1);
			}
			if (Char.myCharz().cMPGoc <= num2 - 2000)
			{
			...
			if (Char.myCharz().cMPGoc <= num2 - 20)
			{
				Service.gI().upPotential(1, 1);
			}
			if (Char.myCharz().cDamGoc <= num3 - 100)
			{
				Service.gI().upPotential(2, 100);
			}
			if (Char.myCharz().cDamGoc <= num3 - 10)
			{
				Service.gI().upPotential(2, 10);
			}
			if (Char.myCharz().cDamGoc <= num3 - 1)
			{
	...
	public void openUIZone1(Message message)
	{
		InfoDlg.hide();
		try
		{
		...
		GameCanvas.panel.setkhu();
		GameCanvas.panel.showkhu();
	}
}
